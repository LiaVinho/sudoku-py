...
# keep drawing squares and moving forward 
def move_turtle():
    turtle.pd()
        ...
        if user_input[0] == grid_rows[i]:
            for j in range(0, len(grid_cols)):
                if user_input[1] == grid_cols[j]:
                    if puzzle[i][j] != 0:  # call the erase entry function and ask
